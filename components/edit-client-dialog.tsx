...
import { ClientRecord, CreateClientRecord, CreateClientRecordSchema } from '@/types/client-record';
...
export function EditClientDialog({ record, open, onOpenChange, onSuccess }: EditClientDialogProps) {
  ...
  const onSubmit = async (data: CreateClientRecord) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/client-records/${record._id}`, {
        method: 'PUT',
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the details for this client energy contract record.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-4">
              <h3 className="font-semibold text-lg text-slate-800">Contact & Additional</h3>
              ...
              <div>
                ...
                <Select
                  onValueChange={(value) => {
                    setValue('status', value as "in progress" | "in discussion" | "completed");
                  }}
                  value={watch('status') || ''}
                >
